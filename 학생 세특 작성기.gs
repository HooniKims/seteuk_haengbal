...
4. **난이도 및 어조 (매우 중요)**:
   ...
   - 학생의 활동을 과장되게 포장하기 위해 어려운 단어를 쓰지 말고, 학생이 실제로 배우고 느낀 점을 담백하고 명확하게 서술하세요.
5. **마침표 및 띄어쓰기 (매우 중요)**:
   - **모든 문장은 반드시 마침표(.)로 끝나야 합니다.**
   - **마침표 뒤에는 반드시 한 칸을 띄우세요.** (예: "...함. 그리고..." O / "...함.그리고..." X)
...
function callOpenAI(activityDescription, rowNumber, grade, subjectName, lengthInstruction) {
  ...
  // 융합 수업 제한 로직 추가
  // 활동 내용에 "융합"이라는 단어가 없으면 융합 관련 내용 금지
  if (!activityDescription.includes('융합')) {
    prompt += `\n\n**중요: 융합 수업 제한**\n활동 내용에 "융합"이라는 단어가 명시되지 않았으므로, **다른 과목과의 융합이나 연계를 절대 언급하지 마세요.** 오직 해당 과목의 특성과 활동 내용에만 집중하여 작성하세요.`;
  } else {
    prompt += `\n\n**참고: 융합 수업**\n활동 내용에 "융합"이 포함되어 있으므로, 과목 간의 연계나 융합적 사고를 적절히 드러내어 작성하세요.`;
  }
